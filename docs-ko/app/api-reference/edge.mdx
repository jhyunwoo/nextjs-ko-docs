---
title: Edge Runtime
description: Edge Runtime에 대한 API 참조입니다.
---

Next.js에는 애플리케이션에서 사용할 수 있는 두 가지 서버 런타임이 있습니다.

- 모든 Node.js API에 접근할 수 있으며 애플리케이션 렌더링에 사용되는 **Node.js Runtime**(기본값)
- 더 제한적인 **Edge Runtime** which contains a more limited [set of APIs](#reference), used in [Proxy](file-conventions/proxy.mdx).

## 주의 사항

- Edge Runtime은 모든 Node.js API를 지원하지 않습니다. 일부 패키지는 예상대로 동작하지 않을 수 있습니다.
- Edge Runtime은 Incremental Static Regeneration (ISR)을 지원하지 않습니다.
- 두 런타임 모두 배포 어댑터에 따라 [streaming](file-conventions/loading.mdx)을 지원할 수 있습니다.
...
| API                                                                             | 설명                        |[`Blob`](https://developer.mozilla.org/docs/Web/API/Blob)                       | Represents a blob                 |
...
| API                                                                                 | 설명                             |[`atob`](https://developer.mozilla.org/en-US/docs/Web/API/atob)                     | Decodes a base-64 encoded string   |
...
| API                                                                                                     | 설명                                 |[`ReadableStream`](https://developer.mozilla.org/docs/Web/API/ReadableStream)                           | Represents a readable stream            |
...
| API                                                                       | 설명                                                                                              |[`crypto`](https://developer.mozilla.org/docs/Web/API/Window/crypto)      | Provides access to the cryptographic functionality of the platform                                  |
...
| API                                                                                                                   | 설명                                                                                                                                     |[`AbortController`](https://developer.mozilla.org/docs/Web/API/AbortController)                                       | Allows you to abort one or more DOM requests as and when desired                                                                                                                                     |
...
| --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `process.env` to access [Environment Variables](../guides/environment-variables.mdx)                                       | 필요할 때 하나 이상의 DOM 요청을 중단할 수 있게 합니다                                                                                  |`next dev` and `next build`.

### Unsupported APIs

|                            | 값의 배열을 나타냅니다                                                                                                                   |
|                | 고정 길이의 범용 원시 이진 데이터 버퍼를 나타냅니다                                                                                      |
...
| [`eval`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/eval)           | 64비트 부호 있는 정수형 typed array를 나타냅니다                                                                                         |
| [`new Function(evalString)`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Function)         | 64비트 부호 없는 정수형 typed array를 나타냅니다                                                                                         |[`WebAssembly.compile`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/WebAssembly/compile)         | Compiles a WebAssembly module from a buffer source                  |
...
| *can not be reached at runtime* and which can not be removed by treeshaking.
You can relax the check to allow specific files with your Proxy configuration:                       | 와  두 값을 가질 수 있는 논리 엔터티를 나타냅니다                                                                           |
|                  |  호출로 설정된 반복 타이머를 취소합니다                                                                                   |
...
`unstable_allowDynamic` 호출로 설정된 타이머를 취소합니다                                                                                         |
| [glob](https://github.com/micromatch/micromatch#matching-features)                                                       | 브라우저 디버깅 콘솔에 접근할 수 있게 합니다                                                                                             |

| *they will throw and cause a runtime error*.                     | 에 대한 범용 뷰를 나타냅니다                                                                                                |
|                              | 플랫폼에 독립적인 형식의 특정 시점을 나타냅니다                                                                                          |
