---
title: 이미지 구성요소
description: 내장된 'next/image' 구성 요소를 사용하여 Next.js 애플리케이션의 이미지를 최적화하세요.
---

Next.js 이미지 구성 요소는 자동 이미지 최적화를 위해 HTML `<img>` 요소를 확장합니다.
...
- [Custom Image Loader Configuration](../config/next-config-js/images.mdx#example-loader-configuration)
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](../../getting-started/images.mdx#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](../config/next-config-js/headers.mdx) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
- SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](../config/next-config-js/headers.mdx#content-security-policy).
...
Use `className` or `style`, not `styled-jsx`. In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](../../getting-started/css.mdx), a [global stylesheet](../../getting-started/css.mdx#global-css), etc.
...
You cannot use [styled-jsx](../../guides/css-in-js.mdx) because it's scoped to the current component (unless you mark the style as `global`).
...
<Image alt="상위 컨테이너의 너비와 높이를 채우는 반응형 이미지" srcLight="/docs/light/responsive-image.png" srcDark="/docs/dark/responsive-image.png" width="1600" height="629" />
...
소스 이미지가 동적 또는 원격 URL인 경우 Next.js가 종횡비를 계산할 수 있도록 너비 및 높이 소품을 제공해야 합니다.
...
이미지의 가로 세로 비율을 모르는 경우 [`fill` 소품](#fill)을 추가할 수 있습니다. 이렇게 하면 이미지가 상위 컨테이너의 전체 너비를 채우게 됩니다.`objectFit` prop set to `cover`. This will make the image fill the full width of its parent container.

<Image alt="상위 컨테이너 너비를 채우는 이미지 그리드" srcLight="/docs/light/fill-container.png" srcDark="/docs/dark/fill-container.png" width="1600" height="529" />
...
이미지가 전체 화면 영역을 덮도록 하려면 `fill` 소품을 사용하십시오.

<Image alt="페이지의 전체 너비와 높이를 차지하는 배경 이미지" srcLight="/docs/light/background-image.png" srcDark="/docs/dark/background-image.png" width="1600" height="427" />
...
다양한 스타일과 함께 사용되는 이미지 구성 요소의 예는 [이미지 구성 요소 데모](https://image-component.nextjs.gallery)를 참조하세요.
...
밝은 모드와 어두운 모드에 대해 다른 이미지를 표시하려면 두 개의 [`width`](image.mdx#width-and-height), [`height`](image.mdx#width-and-height) and optional [`blurDataURL`](image.mdx#blurdataurl) props manually. 구성 요소를 래핑하고 CSS 미디어 쿼리를 기반으로 올바른 구성 요소를 표시하는 새 구성 요소를 생성할 수 있습니다.

사용해 보세요:- `width` and `height` attributes are used to infer the correct aspect ratio of image and avoid layout shift from the image loading in. The `width` and `height` do *not* determine the rendered size of the image file.

To safely allow optimizing images, define a list of supported URL patterns in `next.config.js`, ,  및  소품을 제공할 수 있습니다. .
...
If you want to display a different image for light and dark mode, you can create a new component that wraps two `<Image>` 문자열을  CSS 함수로 변환하여 배경 이미지를 최적화할 수도 있습니다.
...
| [Art Direction](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images#art_direction), you can provide different `src` | `width` 기본 구성이 `height`로 변경됨, `quality` 소품 추가, `getImageProps()` 소품이 더 이상 사용되지 않음,  구성 추가,  구성 추가.                                                                                                                                                                                                                                                                                                            |
...
| `srcSet` | [`image-set()`](https://developer.mozilla.org/en-US/docs/Web/CSS/image/image-set) CSS function to optimize a background image. 구성 기본값이 로 변경되었습니다.                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `v16.1.2` | `maximumResponseBody` 소품이 추가되고 `v16.0.0` 구성이 추가되었습니다.                                                                                                                                                                                                                                                                                                                                                                                                                             |`qualities` default configuration changed to `[75]`, `preload` prop added, `priority` prop deprecated, `dangerouslyAllowLocalIP` config added, `maximumRedirects` config added.                                                                                                                                                                                                                                                                                                            |`v15.3.0`  | `remotePatterns` added support for array of `URL` objects.                                                                                                                                                                                                                                                                                                                                                                                                                                 |`v15.0.0`  | `contentDispositionType` configuration default changed to `attachment`.                                                                                                                                                                                                                                                                                                                                                                                                                    |
